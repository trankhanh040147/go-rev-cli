...
#rv-preset
- [ ] **IS01**  : `~/.config/revcli/presets/` : Do not have custom presets, user can't edit custom presets
- [ ] **IS02** Missing feature let user edit preset (v0.3) in command line or edit file manually 
- [ ] **IS03:** Missing feature let user open folder preset 
- [ ] **IS04** Missing feature so that user can set default preset
- [ ] **IS05**: Preset got overriden by system prompt 
	1. Make the base prompt more generic/neutral (Not just Go reviewer)
	2. Allow presets to optionally replace (not append to) the base prompt
	3. Add a flag like --preset-replace to completely override the base prompt
	4. Allow user to modify system prompt
## New Features 
